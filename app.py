...
def get_uk_boundary():
    """Load a public-domain Natural Earth UK country boundary."""
    response = requests.get(UK_BOUNDARY_URL, timeout=30)
    response.raise_for_status()
    payload = response.json()

    feature = next(
        f for f in payload["features"]
        if f.get("properties", {}).get("ADM0_A3") == "GBR"
        or f.get("properties", {}).get("ISO_A3") == "GBR"
    )
    return feature["geometry"]


def point_is_in_uk(lat, lon, geometry):
    return bool(contains_xy(shape(geometry), lon, lat))


def make_uk_grid():
    """Return only grid-cell centres that fall inside the UK boundary."""
    geometry = shape(get_uk_boundary())
    return [
        (lat, lon)
        for lat, lon in make_grid()
        if geometry.contains(__import__("shapely").geometry.Point(lon, lat))
    ]


def make_heat_surface(map_df, width=520, height=760):
    """
    Turn the modelled cell scores into a smooth RGBA raster and clip it
    exactly to the UK boundary. This avoids the old rectangular red block.
    """
    ...
    points = np.column_stack([map_df["lon"].values, map_df["lat"].values])
    values = map_df["score"].values.astype(float)

    # Interpolate the coarse environmental model onto a smooth raster.
    surface = griddata(points, values, (xx, yy), method="linear")

    # Fill any small edge gaps from the nearest available modelled cell.
    nearest = griddata(points, values, (xx, yy), method="nearest")
    surface = np.where(np.isfinite(surface), surface, nearest)

    # Smooth only the rendered surface — this does not alter the underlying
    # modelled cell values shown in the data table.
    surface = gaussian_filter(surface, sigma=5)
    surface = np.clip(surface, 0, 100)

    geometry = shape(get_uk_boundary())
    uk_mask = contains_xy(geometry, xx, yy)

    # Transparent outside the UK.
    alpha = np.where(uk_mask, 255, 0).astype(np.uint8)

    # Reference-style blue → cyan → green → yellow → orange → red palette.
    ...
    rgba = (cmap(Normalize(0, 100)(surface)) * 255).astype(np.uint8)
    rgba[..., 3] = alpha
...
# ============================================================
    ...
    st.caption(
        "A true UK-shaped environmental heat surface — similar to a "
        "meteorological heat map rather than a grid of coloured squares."
    )
    ...
    if map_df is not None:
        ...
        # Tight UK + Northern Ireland framing.
        m.fit_bounds([[49.7, -10.8], [59.1, 2.2]])

        # The actual heat surface is rendered as an RGBA image and clipped
        # to the UK polygon. This is the key fix for the old rectangle.
        ...
        # Crisp national boundary above the heat layer.
        folium.GeoJson(
            get_uk_boundary(),
            name="UK boundary",
        ...
        # Optional cell boundaries for users who want to inspect model
        # resolution. They remain subtle rather than dominating the heat map.
        if show_grid:
            half = GRID_STEP / 2
            for r in map_df.itertuples():
                folium.Rectangle(
                    ...
                    popup=folium.Popup(
                        f"""
                        <b>Environmental research cell</b><br>
                        Score: <b>{float(r.score):.0f}/100</b><br>
                        Band: <b>{final_band(float(r.score))}</b><br>
                        7-day rainfall: {float(r.rain_7d):.1f} mm<br>
                        Mean temperature: {float(r.temp):.1f}°C
                        """,
                        max_width=260,
                    ),
                ).add_to(m)
