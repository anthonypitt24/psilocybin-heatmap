...
# ============================================================
    ...
    st.caption(
        "A smooth UK-wide environmental suitability surface. "
        "Colours show the modelled index for the selected forecast day."
    )
    ...
    if map_df is not None:
        ...
        # OpenStreetMap is used as the dependable default basemap.
        # The previous CartoDB tiles caused "API KEY REQUIRED" messages.
        m = folium.Map(
            location=[54.3, -3.0],
            zoom_start=5.6,
            tiles="OpenStreetMap",
        ...
        # Keep the initial view tightly around the UK and Ireland.
        m.fit_bounds([[49.7, -10.8], [59.1, 2.2]])

        # Smooth heat surface — deliberately similar to the reference style.
        heat_points = [
        ...
        # Optional subtle grid boundaries make the research resolution visible
        # without destroying the continuous heat-map appearance.
        if view == "Heat map + grid":
            half = GRID_STEP / 2
            for r in map_df.itertuples():
                folium.Rectangle(
                    bounds=[
        ...
        ">
          ...
          <div style="margin-top:8px;opacity:.68;">
            Broad modelled environmental index — not confirmed species presence.
          </div>
        </div>
        """
        m.get_root().html.add_child(folium.Element(legend))

        # A compact UK view works substantially better on phones than the old
        # 720px forced map height.
        st_folium(
            m,
            use_container_width=True,
            height=560,
